...
XCODE_OUTPUT_LIB_PATH="${DERIVED_DATA_PATH}/Build/Products/${CONFIGURATION}/CronetNetworking/libCronetNetworking.a"
RESULT_LIB_PATH="${BASE_DIR}/libCronetNetworking.a"

rm -f $RESULT_LIB_PATH || true
rm -f $XCODE_OUTPUT_LIB_PATH || true

xcodebuild -workspace "${FRAMEWORK_PROJECT_PATH}/CronetFramework.xcworkspace" -scheme CronetFramework -configuration "$CONFIGURATION" -derivedDataPath "$DERIVED_DATA_PATH"
