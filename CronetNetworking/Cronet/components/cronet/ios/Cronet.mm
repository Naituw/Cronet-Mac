...
namespace {
    ...
    id<CRNCertVerifing> gCertVerifier = nil;
    ...
    class ExternalCertVerifier : public net::CertVerifier {
    public:
        explicit ExternalCertVerifier(scoped_refptr<net::CertVerifyProc> verify_proc) : verify_proc_(verify_proc)  {};
        int Verify(const RequestParams& params,
                   net::CRLSet* crl_set,
                   net::CertVerifyResult* verify_result,
                   const net::CompletionCallback& callback,
            ...
            int result = verify_proc_->Verify(params.certificate().get(), params.hostname(), params.ocsp_response(), params.flags(), crl_set, params.additional_trust_anchors(), verify_result);
            
            if (result == net::OK && gCertVerifier) {
                auto cert = params.certificate().get();
                base::ScopedCFTypeRef<CFMutableArrayRef> cert_array(net::x509_util::CreateSecCertificateArrayForX509Certificate(cert));
                SecPolicyRef ssl_policy = SecPolicyCreateSSL(false, nullptr);
                
                if (!cert_array || !ssl_policy) {
                    verify_result->cert_status |= net::CERT_STATUS_INVALID;
                    return net::ERR_CERT_INVALID;
                }
                
                SecTrustRef secTrust = NULL;
                SecTrustCreateWithCertificates(cert_array, ssl_policy, &secTrust);
                ...
                NSString * domain = base::SysUTF8ToNSString(params.hostname());
                BOOL externalResult = [gCertVerifier evaluateServerTrust:secTrust forDomain:domain];
...
(cronet::CronetEnvironment*)cronetEnvironment {
    ...
    if (gMockCertVerifier) {
        gChromeNet.Get()->set_cert_verifier(std::move(gMockCertVerifier));
    }
}
...
+ (void)startInternal {
    ...
    if (gCertVerifier) {
        scoped_refptr<net::CertVerifyProc> defaultVerifyProc = net::CertVerifyProc::CreateDefault();
        std::unique_ptr<net::CertVerifier> cert_verifier = base::MakeUnique<ExternalCertVerifier>(defaultVerifyProc);
        gChromeNet.Get().get()->set_cert_verifier(std::move(cert_verifier));
    }
